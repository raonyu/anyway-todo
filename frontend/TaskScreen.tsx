...
export default function TaskScreen({ activeCategory, categoryConfig }: ScreenProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const formattedData = data.map((item: any) => ({
          id: item.id, title: item.title, memo: item.memo || '', deadlineDate: item.deadline_date || '', deadlineTime: item.deadline_time || '',
  ...
  useEffect(() => { if (hasTime) setDeadlineTime(`${selectedHour}:${selectedMinute}`); }, [selectedHour, selectedMinute, hasTime]);
  ...
  const getFormattedDeadline = (dateStr: string, timeStr: string) => {
    ...
    if (dateStr) {
      const parts = dateStr.match(/(\d+)\.\s*(\d+)\.\s*(\d+)\./);
      if (parts) {
        const y = parseInt(parts[1], 10); const m = parseInt(parts[2], 10); const d = parseInt(parts[3], 10);
        const dateObj = new Date(y, m - 1, d);
        const days = ['일', '월', '화', '수', '목', '금', '토'];
        const dayOfWeek = days[dateObj.getDay()];
        const currentY = new Date().getFullYear();
        const padM = m.toString().padStart(2, '0'); const padD = d.toString().padStart(2, '0');
  ...
  const openAddModal = (quadrant: '당장 해' | '그래도 해' | '해치워' | '나중에 해') => {
    setIsEditMode(false); setCurrentQuadrant(quadrant); setTitle(''); setMemo(''); setDeadlineDate(''); setDeadlineTime('');
    setHasDate(false); setHasTime(false); setTaskStatus('진행 전');
    const today = new Date(); setCurrentYear(today.getFullYear()); setCurrentMonth(today.getMonth() + 1);
    setAddModalVisible(true); setDeadlineVisible(false);
  };

  const openEditModal = (task: Task) => {
    setIsEditMode(true); setEditTaskId(task.id); setCurrentQuadrant(task.quadrant); setTitle(task.title); setMemo(task.memo); 
    setDeadlineDate(task.deadlineDate); setDeadlineTime(task.deadlineTime); setHasDate(!!task.deadlineDate); setHasTime(!!task.deadlineTime);
    setTaskStatus(task.status || (task.isCompleted ? '완료' : '진행 전'));
    if (task.deadlineTime) { const [h, m] = task.deadlineTime.split(':'); if (h && m) { setSelectedHour(h); setSelectedMinute(m); } }
    const today = new Date(); setCurrentYear(today.getFullYear()); setCurrentMonth(today.getMonth() + 1);
    setAddModalVisible(true); setDeadlineVisible(false);
  };
    ...
    try {
      if (isEditMode && editTaskId) {
        ...
        await fetch(`${SERVER_URL}/tasks`, {
          ...
          body: JSON.stringify({ user_id: 1, title, memo, deadline_date: finalDate, deadline_time: finalTime, quadrant: currentQuadrant, is_completed: isCompleted, status: taskStatus, category: activeCategory })
        });
      }
  ...
  const generateCalendar = () => {
    const firstDay = new Date(currentYear, currentMonth - 1, 1).getDay(); const daysInMonth = new Date(currentYear, currentMonth, 0).getDate();
    const days = Array(firstDay).fill(''); for (let i = 1; i <= daysInMonth; i++) days.push(i.toString()); return days;
  };

  const selectDate = (day: string) => { if (day) setDeadlineDate(`${currentYear}. ${currentMonth}. ${day}.`); };
    ...
    return (
      <View style={styles.quadrantBox}>
        <View style={styles.quadrantHeader}>
          <View style={styles.quadrantTitleWrapper}><View style={[styles.dot, { backgroundColor: dotColor }]} /><Text style={styles.quadrantTitle}>{qTitle}</Text></View>
          <TouchableOpacity onPress={() => openAddModal(qTitle)}><Text style={styles.addIcon}>+</Text></TouchableOpacity>
        </View>
          ...
          {quadrantTasks.map(task => {
            ...
            return (
              <View key={task.id} style={styles.taskItem}>
                <TouchableOpacity onPress={() => toggleTaskCompletion(task)} style={styles.checkbox}><Text style={[styles.checkboxText, task.isCompleted && styles.completedColor]}>{task.isCompleted ? '☑' : '☐'}</Text></TouchableOpacity>
                <TouchableOpacity style={styles.taskTextContent} onPress={() => openEditModal(task)}>
                  <Text style={[styles.taskTitleText, task.isCompleted && styles.completedTaskText]}>{task.title}</Text>
                  {qTitle !== '나중에 해' && formattedDeadline ? <Text style={[styles.taskDeadlineText, task.isCompleted ? styles.completedColor : (urgent ? styles.urgentColor : styles.defaultColor)]}>{formattedDeadline}</Text> : null}
                </TouchableOpacity>
              </View>
            );
          })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.matrixContainer}>
        <View style={styles.row}>{renderQuadrant('당장 해', '#ff5a5a')}{renderQuadrant('그래도 해', '#ffb31b')}</View>
        <View style={styles.row}>{renderQuadrant('해치워', '#5a9aff')}{renderQuadrant('나중에 해', '#a0a0a0')}</View>
      </View>
              ...
              <View style={styles.bottomSheet}>
                ...
                <View style={styles.toolbar}>
                  ...
                  <View style={styles.actionGroup}>
                    {isEditMode && (<TouchableOpacity style={styles.actionBtnDel} onPress={deleteTask}><Text style={styles.actionBtnText}>삭제</Text></TouchableOpacity>)}
                    <TouchableOpacity style={[styles.actionBtnSave, { marginLeft: isEditMode ? 10 : 0 }]} onPress={saveTask}><Text style={[styles.actionBtnText, { color: '#fff' }]}>{isEditMode ? '저장' : '추가'}</Text></TouchableOpacity>
                  </View>
                </View>
              </View>
              ...
              <View style={[StyleSheet.absoluteFill, styles.modalOverlayCenter]}>
                <TouchableWithoutFeedback onPress={() => {}}>
                  <View style={styles.deadlineSettingsBox}>
                    <View style={styles.deadlineSettingsHeader}><Text style={styles.deadlineSettingsTitle}>마감일 설정</Text></View>
                    <View style={styles.toggleRow}>
                      ...
                      <Switch trackColor={{ false: "#767577", true: "#5a9aff" }} thumbColor={"#f4f3f4"} value={hasDate} onValueChange={(val) => { setHasDate(val); if (val && !deadlineDate) { const t = new Date(); setDeadlineDate(`${t.getFullYear()}. ${t.getMonth() + 1}. ${t.getDate()}.`); } }} />
                    </View>
